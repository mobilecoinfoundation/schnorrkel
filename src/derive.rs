...
impl ExtendedKey<SecretKey> {
    ...
    pub fn hard_derive_mini_secret_key<B: AsRef<[u8]>>(
        ...
        let (key, chaincode) = self
            .key
            .hard_derive_mini_secret_key(Some(self.chaincode), i);
        ...
        for i in 0..30 {
            ...
            if i % 5 == 0 {
                ...
                assert!(
                    extended_public_key
                        .key
                        .verify(ctx.xof(h.clone()), &good_sig)
                        .is_ok(),
                ...
                assert!(
                    !extended_public_key
                        .key
                        .verify(ctx.xof(h.clone()), &bad_sig)
                        .is_ok(),

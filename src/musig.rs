...
impl Reveal {
    ...
    fn iter_points<'a>(&'a self) -> impl Iterator<Item = CompressedRistretto> + 'a {
        (&self.0)
            .chunks(32)
            .map(|R| CompressedRistretto(array_ref![R, 0, 32].clone()))
    }
    ...
    pub fn add_their_commitment(
        ...
        match self.Rs.entry(them) {
            ...
            Entry::Occupied(o) => {
                if o.get() != &theirs {
                    let musig_stage = MultiSignatureStage::Commitment;
                    return Err(SignatureError::MuSigInconsistent {
                        musig_stage,
                        duplicate: true,
                    });
                }
    ...
    T: SigningTranscript + Clone,
{
    ...
    pub fn cosign_stage(mut self) -> MuSig<T, CosignStage> {
        ...
        *(Rs.get_mut(&self.stage.keypair.borrow().public)
            .expect("Rs known to contain this public; qed")) = CoR::Cosigned { s: s_me.clone() };
        MuSig {
            t,
            Rs,
            stage: CosignStage { R, s_me },
        }
    }
}
...
impl<T: SigningTranscript + Clone> MuSig<T, CosignStage> {
    ...
    pub fn add_their_cosignature(
        ...
        let theirs =
            Scalar::from_canonical_bytes(theirs.0).ok_or(SignatureError::ScalarFormatError)?;
        match self.Rs.entry(them) {
            Entry::Vacant(_) => {
                let musig_stage = MultiSignatureStage::Reveal;
                Err(SignatureError::MuSigAbsent { musig_stage })
...
mod tests {
    ...
    fn aggregation_btreeemap_vs_slice() {
        ...
        debug_assert_eq!(
            btm.public_key(),
            aggregate_public_key_from_slice(vec.as_mut_slice())
                .unwrap()
                .public_key()
        );
